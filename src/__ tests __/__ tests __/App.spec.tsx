...
describe(App, () => {
    ...
    it("Should display the correct column headers", async () => {
        ...
        const expectedHeaders = ['Meeting Name', 'Race Number', 'Start Time', 'Starts in'];
    ...
    it("Should display page title", async () => {
        ...
        const rows = getAllByRole('row').length;
        console.log('length', rows)
        expect(heading).toBe("Next to Go");
    });
});
// expect(tableRows.length - 1).toBe(5);
// const selectCategory = getByTestId('category-filter').textContent
// expect(selectCategory).toStrictEqual("All");
